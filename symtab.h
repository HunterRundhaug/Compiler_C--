#ifndef SYMTAB_H
#define SYMTAB_H

#include <stdio.h>

typedef enum {
    SYM_INT_VAR,      // int variable
    SYM_FUNC      // function
} SymbolType;

// Struct definitions (if not already included elsewhere)

typedef struct Symbol {
    char* name;
    SymbolType type;
    struct Symbol* next;
    struct Symbol* children; // use cases: {(if type is a function, function arguments go here)}
} Symbol;

typedef struct Scope {
    struct Scope* next;
    Symbol* symbols;
} Scope;

// Global scope stack head
extern Scope* table_head;

// Scope management
void add_new_scope();               // Pushes a new scope onto the stack
Symbol* add_new_symbol(char*, SymbolType);    // Pushes a new symbol to the symbol list of the current scope (head of scope)
void pop_current_scope();           // Pops current scope and frees contents
void free_scope(Scope* scope);      // Frees all symbols in a given scope

// Look-ups
int lookup_global_scope(char* name, SymbolType type);
int lookup_global_scope_with_same_type(char* name, SymbolType type);
int lookup_in_current_scope(char* name, SymbolType type);
int in_global_scope();
int lookup_local_to_global(char* name, SymbolType type);

 void append_child_to_symbol(Symbol* parentSymbol, char* Cname, SymbolType Ctype);

#endif